...
def test_order_validation():
    ...
    try:
        ...
        # Test invalid quantity (should still work but log error)
        order = broker.place_order("NIFTY 50", -5, "BUY", 22450)
        assert order is not None  # BrokerInterface doesn't validate
...
def test_error_handling():
    ...
    try:
        ...
        # Test order without connection
        order = broker.place_order("NIFTY 50", 10, "BUY", 22450)
        assert order is None
        
        # Test get_positions without connection
